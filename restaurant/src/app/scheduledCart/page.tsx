...
const ScheduledCart = () => {
  ...
  const totalAmount = scheduledCartItems.reduce((acc, item) => acc + item.price * item.quantity, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 mt-32">
      <h1 className="text-4xl font-extrabold text-gray-900 mb-8 text-center">SCHEDULED ORDER DETAILS</h1>

      {scheduledCartItems.length === 0 ? (
        <div className="text-center text-gray-600">
          <p className="text-lg">Your scheduled cart is empty.</p>
          <Link href="/viewScheduleMenu" className="text-blue-600 mt-4 inline-block text-lg font-semibold hover:underline">
            Continue Scheduling
          </Link>
        </div>
      ) : (
        <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-lg">
          <div className="space-y-6">
            {scheduledCartItems.map((item) => (
              <div key={item.scheduled_cart_id} className="flex items-center space-x-4 p-4 border-b bg-gray-50 rounded-lg shadow-sm">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">Scheduled Date: {new Date(item.scheduled_date).toLocaleDateString()}</p>
                  <p className="text-sm text-gray-600">Scheduled Time: {item.scheduled_time}</p>
                  <p className="text-sm text-gray-600">Service Type: {item.service_type}</p>
                </div>
                <button onClick={() => removeItem(item.scheduled_cart_id)} className="text-red-500 hover:text-red-700 transition">
                  <IoClose size={24} />
                </button>
              </div>
            ))}
      ...
      <div className="flex justify-center mt-8">
        <Link href="/viewScheduleMenu" className="bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold text-lg shadow-md hover:bg-blue-900 transition">
          Add More Items
        </Link>
      </div>
    </div>
