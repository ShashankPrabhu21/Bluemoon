...
const FoodDisplayPage = () => {
  ...
  useEffect(() => {
    if (!foodItemsContainerRef.current || isLoading || error) {
       // Don't start scrolling if container not ready, still loading, or errored
       return;
    }
    ...
    // Reduced scroll duration significantly for faster, more typical TV display scrolling
    ...
    const scroll = (timestamp: number) => {
      ...
      if (targetScroll <= 0) {
         animationFrameId = requestAnimationFrame(scroll); // Keep checking if scroll height becomes positive
         return;
      }
  ...
  }, [foodItemsByCategory, isLoading, error]);
                       ...
                       {foodItemsByCategory[categoryName].map((item) => (
                         ...
                         >
                           <Image
                             ...
                             // Adjust these values based on your exact grid column widths and breakpoints.
